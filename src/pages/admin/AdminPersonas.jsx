...
import { getAllCourses, getUserAssignedCourses, assignCourseToUser, removeCourseFromUser, createUserProfile, updateUserProfile } from '../../lib/db';
...
import { Plus, Pencil, Trash2, X, Check, Search, Calendar, Eye, EyeOff, Copy, RefreshCw, ChevronDown, ChevronUp, AlertCircle, Shield, User } from 'lucide-react';
...
function PersonModal({ person, accounts, courses, onClose, onSave }) {
  ...
  const handleSave = async () => {
    if (!form.displayName || !form.email) { setError('Nombre y correo son requeridos.'); return; }
    setLoading(true);
    try {
      await onSave(person, form, assignedCourseIds, password);
      onClose();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/60 backdrop-blur-sm">
      <div className="glass-strong rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg animate-slide-up max-h-[92vh] flex flex-col">
        <div className="flex items-center justify-between p-5 border-b border-white/5">
          <h3 className="font-display font-semibold text-white">{person ? `✏️ Editar — ${person.displayName}` : '➕ Nueva persona'}</h3>
          <button onClick={onClose} className="text-slate-500 hover:text-slate-200"><X className="w-5 h-5"/></button>
        </div>
          ...
          {sections.map(s => (
            <button key={s.id} onClick={()=>setSection(s.id)}
              className={`px-3 py-1.5 rounded-lg text-xs font-display font-semibold transition-all
                ${section===s.id?'bg-brand-500/15 text-brand-300 border border-brand-500/20':'text-slate-500 hover:bg-white/5'}`}>
              {s.label}
            </button>
          ))}
        </div>
        ...
        <div className="overflow-y-auto flex-1 p-5 space-y-4">
          ...
          {section === 'info' && <>
            <div>
              ...
              <input className="input-field" value={form.displayName} onChange={e=>setForm(f=>({...f,displayName:e.target.value}))} placeholder="Juan García"/>
            </div>
            ...
            <div>
              ...
              <input className="input-field" type="email" value={form.email} onChange={e=>setForm(f=>({...f,email:e.target.value}))} placeholder="correo@gmail.com" disabled={!!person?.uid}/>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
          ...
          {section === 'logistica' && <>
            <div>
              <label className="block text-xs font-mono text-slate-500 mb-2 uppercase tracking-wider">Cuenta logística asignada</label>
            ...
            {!person && (
              <div className="p-3 rounded-xl bg-brand-500/10 border border-brand-500/20 text-brand-300 text-xs">
                💡 La persona quedará asignada al slot disponible de esa cuenta automáticamente.
              </div>
            )}
          </>}
          ...
          {section === 'acceso' && <>
            {!person ? (
              <>
                <div className="p-3 rounded-xl bg-brand-500/10 border border-brand-500/20 text-brand-300 text-xs">
                  ✨ La cuenta de acceso a la plataforma se creará automáticamente.
                </div>
                <div>
                  <label className="block text-xs font-mono text-slate-500 mb-2 uppercase tracking-wider">Contraseña de acceso</label>
                  <div className="relative">
                    <input className="input-field font-mono text-sm pr-24" type={showPass?'text':'password'} value={password} readOnly/>
                    <div className="absolute right-2 top-1/2 -translate-y-1/2 flex gap-1">
                      <button onClick={()=>setShowPass(s=>!s)} className="p-1.5 text-slate-500 hover:text-slate-200">
                        {showPass?<EyeOff className="w-3.5 h-3.5"/>:<Eye className="w-3.5 h-3.5"/>}
                      </button>
                      <button onClick={copyPassword} className={`p-1.5 transition-colors ${copied?'text-jade-400':'text-slate-500 hover:text-brand-400'}`}>
              ...
              <div className="p-3 rounded-xl bg-obsidian-700 text-xs text-slate-400 font-mono">
                Para cambiar la contraseña, ve a Firebase Console → Authentication → el usuario.
              </div>
            )}
            <div>
        ...
        <div className="p-5 border-t border-white/5 flex gap-3 justify-end">
          ...
          <button onClick={handleSave} disabled={loading||!form.displayName||!form.email} className="btn-primary flex items-center gap-2">
...
export default function AdminPersonas() {
  ...
  // Build unified person list from students
  const persons = active.map(st => {
    ...
    return {
      ...
      paid: pay?.paid || false,
      payAmount: pay?.amount || ((st.payments||[]).length > 1 ? 60000 : 80000),
      totalPaid: total,
  ...
  const filtered = persons.filter(p => {
    if (search && !p.displayName?.toLowerCase().includes(search.toLowerCase()) &&
        !p.whatsapp?.includes(search) && !p.email?.toLowerCase().includes(search.toLowerCase())) return false;
    ...
    if (existing) {
      ...
      if (st) {
        ...
        if (form.accountId !== st.accountId) {
          ...
          if (form.accountId) {
            updated.accountId = form.accountId;
            await saveStudent(updated);
            await assignStudentToAccount(accounts, form.accountId, st.id);
          } else {
      ...
      // Create new — Firebase Auth + student record
      ...
      try {
        ...
        // If API fails, still create student record
  ...
  return (
    <div className="flex min-h-screen">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-wrap gap-3 items-center mb-5">
            ...
            <div className="relative flex-1 min-w-48">
              ...
              <input className="input-field pl-11" placeholder="Buscar por nombre, WhatsApp o correo..."
          ...
          {loading ? (
            ...
            <div className="card overflow-hidden p-0 animate-slide-up">
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-white/5">
                      {['Persona','WhatsApp','Cuenta logística','Inicio','Total pagado',`Mes ${m}`,'Acciones'].map(h=>(
                  ...
                  <tbody className="divide-y divide-white/5">
                    {filtered.map(p => (
                      <tr key={p.studentId} className="hover:bg-white/2 transition-colors">
                        ...
                        <td className="px-4 py-3"><PayBadge paid={p.paid} m={m}/></td>
                        <td className="px-4 py-3">
                          <div className="flex gap-1 flex-wrap">
                            ...
                            <button onClick={() => { const st = students.find(s=>s.id===p.studentId); if(st) setPayModal(st); }}
                              className="p-1.5 text-slate-500 hover:text-brand-400 hover:bg-brand-500/10 rounded-lg transition-colors" title="Historial pagos">
                ...
                {filtered.length === 0 && (
                  <div className="text-center py-16 text-slate-500 font-body text-sm">Sin personas</div>
                )}
              </div>
            </div>
          )}
      ...
      {modal && (
        <PersonModal
          person={modal.type === 'edit' ? modal.person : null}
          accounts={accounts}
          courses={courses}
